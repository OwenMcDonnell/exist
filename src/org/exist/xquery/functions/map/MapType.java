...
public class MapType extends AbstractMapType {
    ...
    public void add(final AbstractMapType other) {
        if (other.size() == 1) {
            ...
            if(comparator != null) {
                map = map.__putEquivalent(other.getKey(), other.getValue(), (Comparator)comparator);
            } else {
            ...
            if(other instanceof MapType) {
                if (comparator != null) {
                    transientMap.__putAllEquivalent(((MapType) other).map, (Comparator)comparator);
                } else {
                ...
                for (final Map.Entry<AtomicValue, Sequence> entry : other) {
                    if (comparator != null) {
                        transientMap.__putEquivalent(entry.getKey(), entry.getValue(), (Comparator)comparator);
                    } else {
    ...
    public void add(final AtomicValue key, final Sequence value) {
        ...
        if (comparator != null) {
            map = map.__putEquivalent(key, value, (Comparator)comparator);
        } else {
    ...
    public Sequence get(AtomicValue key) {
        ...
        if (comparator != null) {
            result = map.getEquivalent(key, (Comparator)comparator);
        } else {
            result = map.get(key);
        }
    ...
    public AbstractMapType put(final AtomicValue key, final Sequence value) throws XPathException {
        ...
        if (comparator != null) {
            newMap = map.__putEquivalent(key, value, (Comparator)comparator);
        } else {
    ...
    public boolean contains(AtomicValue key) {
        ...
        if (comparator != null) {
            return map.containsKeyEquivalent(key, (Comparator)comparator);
        } else {
    ...
    public AbstractMapType remove(final AtomicValue key) {
        final Map.Immutable<AtomicValue, Sequence> newMap;
        if (comparator != null) {
            newMap = map.__removeEquivalent(key, (Comparator)comparator);
        } else {
            newMap = map.__remove(key);
        }
    ...
    private void setKeyType(final int newType) {
        if (type == Type.ANY_TYPE) {
            ...
            try {
                ...
                newTransientMap.__putAllEquivalent(map, (Comparator)getComparator(null));   //NOTE: getComparator(null) returns a default distinct values comparator
